...
public class Client {
    ...
    public static void main(String[] args) {
        ...
        if(args.length >= 2) {
            response = runSocket(new RequstEntity(args[0], args[1]), ip, port);
        } else {
            response = runSocket(new RequstEntity("query", "Tux"), ip, port);
        }
     ...
     */
    public static String runSocket(RequstEntity requst, String ip, int port) {
        boolean flag = true;
        String result = "";
    ...
    public static String runSocket(RequstEntity request) {
